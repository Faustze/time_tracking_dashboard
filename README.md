...
## **Деплой на GitHub Pages и статические файлы**

При деплое на GitHub Pages столкнулся с нюансом: файл `data.json`, загружаемый через `fetch`, не попадал в папку `dist/` после сборки Vite. Дело в том, что Vite автоматически обрабатывает только те файлы, которые импортируются в JavaScript напрямую. При использовании `fetch('./data.json')` файл не попадает в бандл — его нужно скопировать вручную.

Решение — добавить шаг копирования в GitHub Actions workflow:

```yaml
- name: Copy data.json to dist
  run: cp src/data.json dist/data.json
```

Также при настройке GitHub Pages через Actions важно не забыть включить сам сервис в настройках репозитория (Settings → Pages → Source: GitHub Actions), иначе деплой завершится ошибкой 404.
